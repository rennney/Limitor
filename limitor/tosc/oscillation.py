import torch
from .io_root import load_eventinfo_tree
...
def prob_oscillation(Etrue, baseline, dm2_41, sin2_2theta_14, sin2_theta_24, mode):
    ...
    Args:
        Etrue (Tensor): neutrino true energy [GeV]
        baseline (Tensor): baseline [km]
        dm2_41 (Tensor): delta m^2_41 [eV^2]
        sin2_2theta_14 (Tensor): sin^2(2θ14)
        sin2_theta_24 (Tensor): sin^2(θ24)
    ...
    y = torch.where(torch.abs(1 - y) < 1e-6, torch.tensor(1.0, device=y.device), y)
